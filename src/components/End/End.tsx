    ...
    return(
        <>
            <div className={`end ${props.className === "hidden" ? "hidden" : "show"}`}>
                <p className="end_text text-center text-3xl pb-5 pt-20">Correct answers - {correct_answers}/10</p>
                {questions.map((question,id) => (
                    <div className={`block_with_info mx-auto flex align-center  justify-center text-center py-5 ${answers[id] === question.correct_answer ? "right" : "wrong" }`}>
                        {/* <div className="block_wrap"> */}
                            <p className="end_text">Question - {question.question}</p>
                            
                            <p className="end_text">Correct answer - {question.correct_answer}</p>
